...
| Service → Appointment                         | Not settled | Not settled: [ADR-0014](../adr/0014-appointment-no-double-booking-invariant.md) (Accepted) explicitly leaves whether an Appointment references a Service unresolved, and the `appointments` table in `db/migrations/0011_appointments.sql` has no `service_id` column — see [`01-entities.md`](./01-entities.md) (Appointment, point 4). |
| StaffMember (practitioner role) → Appointment | 1 to many   | StaffMember is referenced by Appointment; not owned by it.                                                                                                                                                                                                                                                                               |

Every "1 to many" relationship in this table (all except the unresolved Service → Appointment row
above) is read as "one of the left side may be associated with many of the right side, and each
instance of the right side associates with exactly one of the left" — no many-to-many relationship
exists anywhere in this model. A many-to-many would most plausibly arise
...
- An Appointment's practitioner StaffMember must belong to the same Clinic as the Appointment's
  Patient — and if an Appointment references a Service at all (not settled; see the Service →
  Appointment row above and [`01-entities.md`](./01-entities.md)), that Service would need to
  belong to the same Clinic too. Nothing in this model permits assembling an Appointment from
  parts belonging to different clinics.
