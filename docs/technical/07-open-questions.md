...
technical design surfaced while working out *how* to build Deliverable B's domain model. None of
...
| --- | --- | --- |
| **Per-request session variable** (illustrative default used throughout this deliverable) | `SET LOCAL app.current_clinic_id = $1` at the start of each request's transaction; RLS policies read it via `current_setting(...)`. | Simple, requires no schema for identity beyond `staff_sessions`. Fails unsafely (leaks data) if a pooler in *transaction* mode reuses a connection between two requests without resetting the setting — requires either session-mode pooling or explicit reset-on-release, which has real throughput cost. |
...
pipeline's *interface* (P5, already fixed in `05-ai-pipeline.md`) makes swapping the
*implementation* technically straightforward. The one-way cost here is legal/operational, not code
structure.
